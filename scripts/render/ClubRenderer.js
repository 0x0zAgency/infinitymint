...
const pathLocations = {
    Diamond: '/token/Diamond Numbers/',
    Gold: '/token/Gold Numbers/',
    Steel: '/token/Steel Numbers/',
    Wood: '/token/Wood Numbers/',
    Opal: '/token/Opal Numbers/',
};

const backgroundLocations = {
    Diamond: '/token/Diamond_',
    Gold: '/token/Gold_',
    Steel: '/token/Steel_',
    Wood: '/token/Wood_',
    Opal: '/token/Opal_',
...
const registerElements = async (
    ...
    let numberLocation = pathLocations[pathSettings.name] || pathLocations[0];
    let backgroundLocation = backgroundLocations[pathSettings.name] || '';
    let clubNumber;

    if (typeof token.mintData === 'string') {
        if (token.mintData.startsWith('0x')) {
            let result = controller.web3.utils.toAscii(token.mintData);
        ...
        } else clubNumber = token.mintData;
    } else clubNumber = token.mintData?.digit;

    clubNumber = parseInt(clubNumber || Math.floor(100 + Math.random() * 9999));

    let locationKey = '999';
    if (clubNumber <= 999) backgroundLocation += '999.png';

    if (clubNumber > 999 && clubNumber <= 9999) {
        backgroundLocation += '10K.png';
        locationKey = '10K';
    }

    if (clubNumber > 9999 && clubNumber <= 99999) {
        backgroundLocation += '100K.png';
        locationKey = '100K';
    }

    let overlayLocation = '/token/Overlay_' + locationKey + '.png';
    ...
    numbers = numbers.map((number) => {
        return numberLocation + number + '.png';
    });

    let numberImages = await Promise.all(
    ...
    if (settings.forceBackground !== undefined)
        ...
        image = await getImage(backgroundLocation);
    }
    ...
    registerElement(id, await getImage(overlayLocation), {
        onRender: (element, canvas) => {
            setElementPadding(element, token.pathId, controller, canvas);
            if (pathSettings.translate !== undefined) {
                element.set(
    ...
    for (let i = 0; i < numberImages.length; i++) {
        let numberImage = numberImages[i];
        let shrink = locationKey === '999' ? 3 : 3.5;
        if (locationKey === '100K') shrink = 4;

        registerElement(id, numberImage, {
            onRender: (element, canvas) => {
                //set the position origin
                element.originX = 'center';
                element.originY = 'center';

                let location = numberLocations[locationKey][i] || {
                    x: 100 * i,
                    y: 100 * i,
                };
                ...
                let yPercent = (location.y / 100) * canvas.height;
                let xPercent = (location.x / 100) * canvas.width;

                element.top = canvas.height / 2 + yPercent;
                element.left = canvas.width / 2 + xPercent;
                element.set('scaleX', canvas.width / 1000 / shrink);
                element.set('scaleY', canvas.height / 1000 / shrink);
    ...
    registerElement(id, await getImage(settings?.logo || InfinityMintLogo), {
        onRender: (element) => {
            //set the position origin
            element.originX = 'center';
            element.originY = 'center';
            ...
            element.opacity = settings.logoOpacity || 0.25;
            //does stuff
            element.setCoords();

            element.on('mousedown', () => {
                if (settings.save) settings.save(canvas, selectedStickers);
                selectedStickers = [];
            });
        },
        onInitialize: (element) => {},
...
const ClubRenderer = {
    ...
    postRenderToken: async (
        ...
        if (
            ...
            try {
                if (Elements[renderedToken.props.id] !== undefined) {
                    for (
                        let i = 0;
                        i < Elements[renderedToken.props.id].length;
                        i++
                    ) {
                        let element = Elements[renderedToken.props.id][i];
                        if (element.onRemove)
                            element.onRemove(element.element, canvas);
    ...
    updateToken: async (
        ...
        if (SpawnedInstances[renderedToken.props.id] !== undefined) {
            ...
            for (let i = 0; i < elements.length; i++) {
                let element = elements[i];
                if (element.onRemove) element.onRemove(element.element, canvas);
            ...
            //render again
            await ClubRenderer.postRenderToken(
                controller,
                renderedToken,
                token,
                stickers,
    ...
    createTokenURI: async (
        ...
        attributes.push({
            trait_type: 'Material',
            value: pathSettings.name || token.pathId,
        });
        ...
        let description = `A special membership token verifying that ${clubNumber}.eth is apart of the ${club} Club. Its got a material finish of ${
            controller.getPathSettings(token.pathId).name
        }. Have you got your ${club} Club membership token? Get one now at ${
            Config.settings.url
        }/mint`;

        return {
            name: token.name + ' - ' + clubNumber + '.eth',
