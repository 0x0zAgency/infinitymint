      ...
      <Modal.Body>
        <div className="d-grid gap-2">
          <Alert hidden={values.image?.length !== 0}>
            <p>
              Your branch is going to need an image in order to look fancy and
              stuff.
            </p>
            <Button
              varaint="success"
          ...
          {values?.data?.key ? (
            <p className="display-2 neonText">{values?.data?.key}</p>
          ) : (
            <></>
          )}

          <Form.Group className="mb-2" controlId="minterName">
            <Form.Label className="fs-5">New Element Key</Form.Label>
            <Form.Control size="sm" ref={keyElement} />
          </Form.Group>
          <Button
            variant="success"
            onClick={() => {
              if (
                ...
                images.filter(
                  (thatImage) => thatImage === keyElement.current?.value
                ).length !== 0
              )
                return;
          ...
          <Button
            ...
            onClick={() => {
              ...
              setValues({
                ...
                data: {
                  ...values.data,
                  images: [...images, keyElement.current?.value],
                },
              });
            }}
          >
                  ...
                  <div className="mt-2">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        setValues(_values);
                        setImages(images.filter((i) => i !== image?.key));
                      }}
        ...
        <Form className="mt-2">
          {Object.keys(values)
            ...
            .map((key) => {
              if (images.filter((thatImage) => thatImage === key).length !== 0)
                return <></>;

              return (
                <Form.Group className="mb-1" controlId="minterName">
                  <Form.Label className="fs-5">{key}</Form.Label>
                  <Form.Control
