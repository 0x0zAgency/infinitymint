...
def convert_to_16bit(image_path, output_folder, tint_factor):
    # Open the image
    image = Image.open(image_path)
    
    # Reduce resolution to emulate pixelation
    small = image.resize((int(image.width / 4), int(image.height / 4)), resample=Image.BILINEAR)
    
    # Restore to original size
    image = small.resize(image.size, Image.NEAREST)
    
    # Quantize to 16-bit colors
    ...
    output_path = os.path.join(output_folder, "16bit_" + os.path.splitext(filename)[0] + ".png")
...
def process_folder(source_folder, output_folder, tint_factor):
    ...
    for img_file in image_files:
        ...
        convert_to_16bit(img_path, output_folder, tint_factor)

# Example usage
# Adjust 'source_folder', 'results', and tint_factor as needed.
process_folder('/Users/0xwizardof0z/Documents/GitHub/infinitymint-alpha/imports/goodies/imnotart', '/Users/0xwizardof0z/Documents/GitHub/infinitymint-alpha/imports/goodies/imnotart', -0.5)
