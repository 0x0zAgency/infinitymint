...
Controller.deployConfig = {
	...
	defaultLinks: [
		...
		// link index 1 (can also reference by index)
		{
			...
			onlyForced: false, //can only be set via another contract other than the linker
			description:
			"Allows your token access to the Ethereum Advertising Service, a decentralized monetization protocol that supports ontoken advertising and sponsorship models. One of the products are called the EADS Stickers which enables virtual stickers to be placed onto your tokenURI for a price that you set.",
	...
	networks: {
		...
		polygonzkevmtestnet: {
			...
			gasStation:
				"https://api.etherscan.io/api?module=gastracker&action=gasoracle&apikey=YZRHB3P4MXVSJ9S96F34UUR9DVV4QFZ2MC", //gas station location
			showGas: false, //show us how much gas is before we do anything
			gasValue: 1e9, //the number of digits in a gwei
			priceStation:
				"https://api.etherscan.io/api?module=stats&action=ethprice&apikey=YZRHB3P4MXVSJ9S96F34UUR9DVV4QFZ2MC",
			onGetPrice: (tokenPrice, thisObject) => {
		...
		basegoerli: {
			...
			gasValue: 1e9,
			priceStation:
				"https://api-goerli.basescan.org/api?module=stats&action=ethprice&apikey=4JGA6P1JEYE9SXMW8P6TZYJ7CJ6U3SYXHE",
			onGetPrice: (tokenPrice, thisObject) => {
				if (tokenPrice.status !== "1")
					return {
						price: 1000,
